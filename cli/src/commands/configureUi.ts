...
import { mkdir, readFile as readFileFromFs, writeFile } from "node:fs/promises";
...
type MaintenanceJobKind = "idle" | "update" | "fix";
...
let maintenanceJob: MaintenanceJobState = {
  running: false,
  completed: false,
  ok: false,
  kind: "idle",
...
type SetupResultKind = "onething" | "feishu";
...
function getOpenClawSetupResultPath(kind: SetupResultKind): string {
  const home = resolveHomeDir();
  const file = kind === "onething" ? "onething-setup-result.json" : "feishu-setup-result.json";
  return join(home, ".openclaw", file);
}

function getOpenClawDotEnvPath(): string {
...
function maskSecret(value: string, head = 3, tail = 3): string {
  const v = value ?? "";
...
function upsertDotEnvVar(envText: string, key: string, value: string): string {
...
async function resolveProjectRoot(): Promise<string> {
  ...
  for (const candidate of candidates) {
    if ((await pathExists(join(candidate, "package.json"))) && (await pathExists(join(candidate, "automation")))) {
...
function prependExistingBinDirs(dirs: string[]): void {
  const current = process.env.PATH ?? "";
...
function addCommonUserBinDirsToPath(): void {
  const home = process.env.HOME ?? "";
...
async function refreshProcessPathFromLoginShell(): Promise<void> {
  if (process.platform === "win32") {
    return;
  }
  const shell = process.env.SHELL && process.env.SHELL.trim().length > 0 ? process.env.SHELL : "/bin/zsh";
  const refreshed = await runCommand(shell, ["-lc", "printf '%s' \"$PATH\""]);
  if (refreshed.code === 0 && refreshed.stdout.trim()) {
    process.env.PATH = refreshed.stdout.trim();
  }
}
...
async function runAction(action: string, payload: Record<string, string>): Promise<ActionResponse> {
  ...
  const startDetached = async (cmd: string, args: string[], cwd?: string): Promise<{ ok: boolean; error?: string }> => {
  ...
  const startAutomationDetached = async (
    cmd: string,
    args: string[],
    cwd?: string,
    ...
    const logPath = join(
      tmpdir(),
      `clawwrapper-automation-${Date.now()}-${Math.random().toString(36).slice(2, 10)}.log`,
    );
    let logFd: number;
    try {
    ...
    const readLogTail = (): string => {
      try {
        const raw = readFileSync(logPath, "utf8").trimEnd();
        if (!raw) {
          return "";
        }
    ...
    const endLogSession = (keepLogFile: boolean): void => {
      closeLogFdOnce();
      if (!keepLogFile) {
        unlinkLogFile();
      }
    ...
    return await new Promise((resolve) => {
      ...
      const failWithTail = (headline: string): void => {
  ...
  const installPlaywrightChromiumForProject = async (
    projectRoot: string,
    pnpmCmd: string,
    npxCmd: string,
    hasPnpm: boolean,
    ...
    const summarize = (r: { stderr: string; stdout: string }): string =>
      [r.stderr, r.stdout].map((s) => s.trim()).filter(Boolean).join("\n");

    const first = hasPnpm
      ? await runCommand(pnpmCmd, pnpmInstall, { cwd: projectRoot, env })
      : await runCommand(npxCmd, npxInstall, { cwd: projectRoot, env });
    if (first.code === 0) {
      return { ok: true, detail: "" };
    }
    const firstLog = summarize(first);
    if (hasPnpm && hasNpx) {
      const second = await runCommand(npxCmd, npxInstall, { cwd: projectRoot, env });
      if (second.code === 0) {
        return { ok: true, detail: "" };
      }
      return {
  ...
  switch (action) {
    ...
    case "fix": {
      ...
      const kind = action as "update" | "fix";
      const syncJob: MaintenanceJobState = {
        running: false,
        completed: true,
        ok: false,
        kind,
    ...
    case "feishuSetup": {
      ...
      const npxPlaywrightCheck = hasNpx
        ? await runCommand(npxCmd, ["playwright", "--version"], { cwd: projectRoot, env: pwEnv })
        : { code: 1, stdout: "", stderr: "" };
      ...
      const chromiumInstall = await installPlaywrightChromiumForProject(
        projectRoot,
        pnpmCmd,
        npxCmd,
        hasPnpm,
      ...
      if (!chromiumInstall.ok) {
        const response = {
          ...
          title: "Feishu 配置启动失败",
          logs:
            "安装 Playwright Chromium 失败。默认使用 Playwright 官方多 CDN；若需国内镜像可设置 CLAW_WRAPPER_PLAYWRIGHT_DOWNLOAD_HOST，或 CLAW_WRAPPER_PLAYWRIGHT_USE_NPPMIRROR=1（镜像可能滞后导致 404）。\n" +
      ...
      const response = {
        ...
        logs:
          `已启动独立 Playwright Chromium 窗口（飞书专用临时配置目录，与 OneThing 互不共享）。botName=${botName}\n` +
    ...
    case "oneThingSetup": {
      ...
      const response = {
        ...
        logs:
          "已启动独立 Playwright Chromium 窗口（OneThing 专用临时配置目录，与飞书互不共享）。注册/登录后将尝试进入 API Keys 并创建 Key。\n" +
          `结果文件: ${outputPath}`,
      };
      debugLog("action-finish", { action, ok: response.ok });
      return response;
...
async function getOpenClawBinaryCandidates(): Promise<string[]> {
  const home = process.env.HOME ?? "";
  ...
  if (await commandExists("npm")) {
    const npmCmd = process.platform === "win32" ? "npm.cmd" : "npm";
    const prefix = await runCommand(npmCmd, ["config", "get", "prefix"]);
...
async function resolveOpenClawCommand(): Promise<string | null> {
...
async function isOpenClawBinaryUsable(): Promise<boolean> {
  ...
  const probe = await runCommand(openclawCmd, ["--version"]);
...
async function isDaemonReady(binaryUsable: boolean): Promise<boolean> {
  ...
  const status = await runCommand(openclawCmd, ["status"]);
...
function appendMaintenanceLog(text: string): void {
  const normalized = text.trim();
  if (!normalized) {
    return;
  }
...
async function appendNpmDebugLogTailToAppender(output: string, appendLog: (line: string) => void): Promise<void> {
...
async function runMaintenancePipelineOnJob(
  job: MaintenanceJobState,
  kind: "update" | "fix",
  appendLog: (line: string) => void,
  ...
  if (kind === "update") {
    appendLog(`使用镜像源: ${DEFAULT_NPM_REGISTRY}`);
    const mirrorEnv = withMirrorRegistry();
    const npmCmd = process.platform === "win32" ? "npm.cmd" : "npm";
    const ok = await runMaintenanceStep(
      npmCmd,
      ["install", "-g", "openclaw@latest", "--verbose", "--progress=true"],
      job,
      appendLog,
      { env: mirrorEnv, stepName: "npm install -g openclaw@latest", inactivityTimeoutMs: 10 * 60 * 1000 },
    );
...
function startMaintenanceJob(kind: "update" | "fix"): boolean {
...
function startInstallJob(): boolean {
  ...
  void (async () => {
    ...
    const stableSource = await ensureStableOpenClawSource(appendInstallLog);
    if (!stableSource.ok) {
      installJob.running = false;
      installJob.completed = true;
      installJob.ok = false;
      installJob.title = "安装失败（缺少本地源码）";
      installJob.lastFailureReason = stableSource.message ?? `未找到本地源码: ${LOCAL_OPENCLAW_SOURCE_RELATIVE}`;
      appendInstallLog(stableSource.message ?? `未找到本地源码目录 ${LOCAL_OPENCLAW_SOURCE_RELATIVE}。`);
...
export async function runConfigureUiCommand(options: ConfigureUiCommandOptions): Promise<number> {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === "GET" && req.url === "/automation/onething/status") {
      const result = await tryReadJsonFile<any>(getOpenClawSetupResultPath("onething"));
      ...
      const apiKey =
        typeof result?.captured?.apiKey === "string" ? (result.captured.apiKey as string).trim() : "";
      const success = apiKey.length > 0;
      const tailSteps: string[] = Array.isArray(result?.steps) ? (result.steps as string[]).slice(-10) : [];
    ...
    if (req.method === "POST" && req.url === "/automation/onething/apply") {
      try {
        ...
        let envText = "";
        try {
          envText = await readFileFromFs(envPath, "utf8");
        } catch {
          envText = "";
        }
    ...
    if (req.method === "GET" && req.url === "/automation/feishu/status") {
      ...
      const webhookUrl =
        typeof result?.captured?.webhookUrl === "string" ? result.captured.webhookUrl.trim() : "";
