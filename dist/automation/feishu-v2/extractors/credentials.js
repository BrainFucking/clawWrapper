...
exports.extractCredentialsFromText = extractCredentialsFromText;
exports.extractCredentialsFromPage = extractCredentialsFromPage;
function extractByRegex(text, regex) {
    const match = text.match(regex);
    return match?.[1]?.trim();
}
function extractCredentialsFromText(pageUrl, bodyText) {
    const appId = extractByRegex(pageUrl, /(cli_[a-zA-Z0-9]+)/) ??
...
async function extractCredentialsFromPage(page) {
    const appIdText = (await readTextBySelectors(page, [
        '[data-testid="app-id"]',
        'code:has-text("cli_")',
        'text=/App ID|应用 ID|应用ID/i',
    ])) ?? "";
    const appSecretText = (await readTextBySelectors(page, [
    ...
    const webhookUrl = extractByRegex(webhookText, /(https:\/\/open\.feishu\.cn\/open-apis\/bot\/v2\/hook\/[a-zA-Z0-9_\-]+)/i) ?? fallback.webhookUrl;
