...
async function countVisibleBySelectors(page: any, selectors: string[]): Promise<number> {
  let count = 0;
  for (const selector of selectors) {
    try {
...
async function closeAnnouncementPopupBestEffort(page: any, steps: string[]): Promise<void> {
...
async function fillCreateKeyNameBestEffort(page: any, value: string, steps: string[]): Promise<boolean> {
  const selectors = [
  ...
  try {
    const filled = await page.evaluate((nextValue: string) => {
...
async function waitForKeyRow(page: any, keyName: string, timeoutMs: number): Promise<boolean> {
...
async function captureCreateKeyDebug(page: any, steps: string[], label: string): Promise<void> {
  try {
    const debug = await page.evaluate(() => {
      ...
      const inputs = Array.from(document.querySelectorAll("input, textarea")).map((node) => ({
        ...
        value: (node as HTMLInputElement | HTMLTextAreaElement).value || "",
      }));
...
async function submitCreateKeyBestEffort(page: any, steps: string[]): Promise<boolean> {
  for (const selector of [
...
async function clickConsoleEntryWithLabel(page: any, steps: string[], label: string): Promise<boolean> {
...
async function switchLoginToWechatBestEffort(page: any, steps: string[], label: string): Promise<boolean> {
  for (const selector of [
  ...
  try {
    const clicked = await page.evaluate(() => {
      const nodes = Array.from(document.querySelectorAll("button, [role='button'], span")) as HTMLElement[];
      const target = nodes.find((node) => (node.innerText || node.textContent || "").includes("微信登录"));
      if (!target) return false;
      const clickable = (target.closest("button, [role='button']") as HTMLElement | null) || target;
      clickable.click();
      return true;
    });
    if (clicked) {
...
function shouldTryWechatSwitch(bodyLower: string): boolean {
  const hasWechatEntry = bodyLower.includes("微信登录") || bodyLower.includes("wechat");
  const alreadyInWechatQrFlow =
    bodyLower.includes("微信扫码") ||
...
async function captureApiKeyFromRecentRow(page: any, keyName: string, steps: string[]): Promise<string> {
  if (!keyName) return "";
  try {
    const tokenFromRow = await page.evaluate(async (targetName: string) => {
      ...
      const rows = Array.from(document.querySelectorAll("tr, .table-row, [role='row']")) as HTMLElement[];
      ...
      const tokenEl = row.querySelector("[data-clipboard-text], [data-copy], code, input, textarea") as
        | HTMLElement
        | HTMLInputElement
        | HTMLTextAreaElement
        | null;
      if (tokenEl) {
        const attrText =
          tokenEl.getAttribute?.("data-clipboard-text") ||
      ...
      const copyTargets = Array.from(
        row.querySelectorAll("button, a, span, div, i, svg"),
      ) as HTMLElement[];
      for (const target of copyTargets) {
        ...
        if (!marker.includes("复制") && !marker.includes("copy")) continue;
        target.click();
...
async function captureApiKeyBestEffort(page: any, steps: string[], keyName = ""): Promise<string> {
  ...
  try {
    const candidate = await page.evaluate(() => {
      const nodes = Array.from(document.querySelectorAll("input, textarea")) as Array<HTMLInputElement | HTMLTextAreaElement>;
...
async function captureClipboardApiKeyBestEffort(page: any, steps: string[], label: string): Promise<string> {
  try {
    const clip = await page.evaluate(async () => {
      try {
        return await navigator.clipboard.readText();
...
async function refreshKeyListBestEffort(page: any, steps: string[]): Promise<void> {
  for (const selector of [
    'button:has-text("刷 新")',
    'button:has-text("刷新")',
...
async function hasCreateKeyEditorOpen(page: any): Promise<boolean> {
  const selectors = [
    '[role="dialog"] input',
...
async function waitForCreateKeyEditor(page: any, timeoutMs: number): Promise<boolean> {
  const started = Date.now();
  while (Date.now() - started < timeoutMs) {
...
async function clickCreateKeyPlusIconBestEffort(page: any, steps: string[]): Promise<boolean> {
  for (const selector of [
  ...
  try {
    const clicked = await page.evaluate(() => {
      const icon = document.querySelector('span[role="img"][aria-label="plus"], .anticon.anticon-plus') as HTMLElement | null;
      if (!icon) return false;
      const clickable = (icon.closest("button, [role='button'], a, .ant-btn") as HTMLElement | null) || icon;
      clickable.click();
      return true;
    });
    if (clicked) {
...
async function clickCreateApiKeyExactSpanBestEffort(page: any, steps: string[]): Promise<boolean> {
  for (const selector of [
...
async function clickCreateKeyNamedButtonBestEffort(page: any, steps: string[]): Promise<boolean> {
  for (const selector of [
...
async function clickCreateApiKeyIfVisible(page: any, createKeySelectors: string[], steps: string[]): Promise<boolean> {
  try {
...
async function clickByTextIfVisible(page: any, textRegex: RegExp, steps: string[], label: string): Promise<boolean> {
  try {
...
function isOnethingLoginPageText(bodyLower: string): boolean {
  return (
    bodyLower.includes("密码登录") ||
...
async function waitUntilNotLoginPage(
  page: any,
  timeoutMs: number,
  steps: string[],
  pollMs = 2000,
...
export async function runOneThingApiKeySetup(
  ...
  const userDataDir = path.join(
    tmpdir(),
    `clawwrapper-onething-${Date.now()}-${process.pid}`,
  );
  ...
  try {
    ...
    if (clickedCreate) {
      ...
      const submitted =
        (await clickFirstVisible(page, confirmSelectors)) || (await submitCreateKeyBestEffort(page, steps));
      await delay(1500);
      if (submitted) {
